function CacheTL(timeLimit, keyMode) {
    ...
    keyMode键模式，为真时，表示仅存储键，不存储值（不使用this.data）。
...
CacheTL.prototype.clear = function() {
    ...
    for (var key in this.hash) {
        if ((time - this.hash[key]) > this.timeLimit) {
...
CacheTL.prototype.get = function(key) {
    ...
    if (this.hash[key] !== undefined) {
        ...
        else return JSON.parse(this.data[key].toString());
    } else return null;
};
...
CacheTL.prototype.info = function() {
    var len = 0, size = 0;
    if (this.keyMode) {
        len = Object.keys(this.hash).length;
    } else {
        for (var key in this.hash) {
            len +=1;
...
// test:
/*var user = new CacheTL(5 * 1000, 2); //时间限制为5秒钟，5秒前的缓存内容将被删除
var time = Date.now();
user.put('user', {
    name: 'admin',
    age: 30
