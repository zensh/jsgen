...
function pagination(req, list, cache, callback) {
    ...
    if (result.pagination.total <= result.pagination.num) delete result.pagination;
    list.reverse();
    next();
...
function checkTimeInterval(req, type, dm) {
    ...
    if (jsGen.cache.timeInterval.get(req.session._id + type)) return true;
};
