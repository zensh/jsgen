...
function connect() {
    ...
    return then.each([globalCacheDb, onlineCacheDb, onlineUserCacheDb], function (defer, x) {
        client[x] = redis.createClient(redisPort, redisIp, {
            connect_timeout: 5000
        }).on('ready', function () {
            then(function (defer2) {
                client[x].select(x, defer2);
            }).then(function () {
                console.log('Redis connected: ' + redisIp + ':' + redisPort + ', DB: ' + x);
                defer(null);
            }).fail(function () {
                defer('Redis connect failed: ' + redisIp + ':' + redisPort + ', DB: ' + x);
            });
        });
    });
}
